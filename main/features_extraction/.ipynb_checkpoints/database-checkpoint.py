...
class OUR_database():
    """
    OUR database related explanations
    """
    def __init__(self, database_dir, emot_map = {'Angry': 0, 'Sad':1, 'Happy':2, 'Neutrality':3, 'Anxious':4, 'Hurt':5, 'Embarrassed':6}, include_scripted=False): 
        print("dir = ", database_dir)
        #Path
        self.database_dir = database_dir
        #Emotion to label mapping for features
        self.emot_map = emot_map
        #OUR available emotion classes 
        self.all_emo_classes = OUR_EMO_CODES.keys()
    ...
    def get_files(self):
        """
        Get all the required .wav file paths
        """
        ...
        for wav_file in os.listdir(dataset_dir):
            ...
            if ext != ".wav":
                continue
            #Read corresponding json file for the emotion label
            json_file = os.path.join(dataset_dir, name + ".json")
            ...
            label = self.emot_map[emotion]
            all_files.append((os.path.join(dataset_dir, wav_file), label))
          
        return all_files

SER_DATABASES = {'OUR': OUR_database}   
