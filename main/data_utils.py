...
class TestDataset(torch.utils.data.Dataset):
    ...
    def confusion_matrix_iemocap(self, utt_preds):
        ...
        conf_fmt = pd.DataFrame({"Sad": conf[:, 0]})
        # conf_fmt = pd.DataFrame({"Ang": conf[:, 0], "Sad": conf[:, 1],
        #              "Happy": conf[:, 2], "Neutrality": conf[:, 3], "Anxious": conf[:, 4], "Hurt": conf[:, 5], "Embarrassed": conf[:, 6]})
...
class SERDataset:
    ...
    def __init__(self, features_data, num_classes = 7,
                train_ratio=0.6,val_ratio=0.2,oversample=False):
        ...
        if train_mfcc_data is None:
            train_spec_data = features_data['seg_spec'].astype(np.float32)
            train_mfcc_data = features_data['seg_mfcc'].astype(np.float32)
            train_audio_data = features_data['seg_audio'].astype(np.float32)
        else:
            train_spec_data = np.concatenate((train_spec_data, 
                                        features_data['seg_spec'].astype(np.float32) ),
                                        axis=0)
            train_mfcc_data = np.concatenate((train_mfcc_data, 
                                        features_data['seg_mfcc'].astype(np.float32) ),
                                        axis=0)
            train_audio_data = np.concatenate((train_audio_data, 
                                        features_data['seg_audio'].astype(np.float32) ),
                                        axis=0)

        #Concatenate the corresponding labels
        if train_seg_labels is None:
            train_seg_labels = features_data['seg_label'].astype(np.long)
            train_labels = features_data['utter_label'].astype(np.long)
        else:
            train_seg_labels = np.concatenate((train_seg_labels,
                                           features_data['seg_label'].astype(np.long)),
                                           axis=0)
            train_labels = np.concatenate((train_labels,
                                           features_data['utter_label'].astype(np.long)),
                                           axis=0)
        ...
        self.val_spec_data  = features_data['seg_spec'].astype(np.float32)
        self.val_mfcc_data  = features_data['seg_mfcc'].astype(np.float32)
        self.val_audio_data  = features_data['seg_audio'].astype(np.float32)
        self.val_seg_labels = features_data['seg_label'].astype(np.long)
        self.val_labels     = features_data['utter_label'].astype(np.long)
        self.val_num_segs   = features_data['seg_num']
        ...
        self.test_seg_labels = features_data['seg_label'].astype(np.long)
        self.test_labels     = features_data['utter_label'].astype(np.long)
