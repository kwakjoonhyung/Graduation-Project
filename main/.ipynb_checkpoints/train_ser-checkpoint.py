        ...
        with tqdm(train_loader) as td:
            for train_batch in td:
                ...
                if epoch == params['num_epochs']-1:
                    ...
                    mfcc_result = outputs['audio_mfcc'].cpu().detach().numpy()
                    print('mfcc_result shape : ',mfcc_result.shape[0])
                    for row1 in mfcc_result:
                        mfcc_results.append(row1)
                    df = pd.DataFrame(mfcc_results)
                    # Write the DataFrame to a CSV file in append mode
                    df.to_csv(f'audio_mfcc.csv', mode='a', header=False, index=False)
                    
                    # get the attention_weights result
                    attention_weight = f.softmax(outputs['attention_weight'], dim=2)
                    # Assuming attention_weights is a PyTorch tensor, convert it to a NumPy array
                    attention_weights_np = attention_weight.squeeze(1).detach().cpu().numpy()
                    attention_weights_np = attention_weights_np.reshape(1, -1)
                    print('attention_weights_np shape : ',attention_weights_np.shape[0])
                    for row2 in attention_weights_np:
                        attention_weights.append(row2)
                    df = pd.DataFrame(attention_weights)
                    # Write the DataFrame to a CSV file in append mode
                    df.to_csv(f'new_attention_weights.csv', mode='a', header=False, index=False)

#                     now = datetime.datetime.now()
#                     current_time = now.strftime("%Y%m%d%H%M%S")

#                     # Create an array of time steps
#                     time_steps = np.arange(len(example_attention))

#                     # Create a new figure
#                     fig = plt.figure(figsize=(10, 5))

#                     # Add a subplot for the heatmap (1 row, 2 columns, position 1)
#                     ax1 = fig.add_subplot(1, 2, 1)
#                     img = ax1.imshow(example_attention[np.newaxis, :], aspect='auto', cmap='viridis')
#                     ax1.set_xlabel('Time steps')
#                     ax1.set_yticks([])
#                     fig.colorbar(img, ax=ax1, label='Attention weights')

#                     # Add a subplot for the bar graph (1 row, 2 columns, position 2)
#                     ax2 = fig.add_subplot(1, 2, 2)
#                     ax2.bar(time_steps, example_attention)
#                     ax2.set_xlabel('Time steps')

#                     # Save the figure as an image with the current date and time in the filename
#                     plt.savefig(f'new_img/attention_{current_time}_example_{example_idx}.png', dpi=300, bbox_inches='tight')
...
#     print("original wav : ", outputs['audio_wav_org'])
#     print("attention wav : ", outputs['audio_wav_att'])  
    
            
#     # attention 된 wav tensor를 csv에 저장    
#     audio_wav_att = outputs['audio_wav_att'].cpu().detach().numpy()

#     if os.path.isfile('audio_wav_att.csv'):
#         with open('audio_wav_att.csv', 'a') as file2:
#             np.savetxt(file2, audio_wav_att, delimiter=',')
#     else:
#         np.savetxt('audio_wav_att.csv', audio_wav_att, delimiter=',')
        
#     # 원본 wav tensor를 csv에 저장
#     audio_wav_org = outputs['audio_wav_org'].cpu().detach().numpy()

#     if os.path.isfile('audio_wav_org.csv'):
#         with open('audio_wav_org.csv', 'a') as file1:
#             np.savetxt(file1, audio_wav_org, delimiter=',')
#     else:
#         np.savetxt('audio_wav_org.csv', audio_wav_org, delimiter=',')attention_weight
