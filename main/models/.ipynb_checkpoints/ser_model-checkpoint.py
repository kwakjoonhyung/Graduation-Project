...
class Ser_Model(nn.Module):
    def __init__(self):
        ...
        self.post_mfcc_layer = nn.Linear(1017344, 128) # 40 for attention and 8064 for conv, 32768 for cnn-lstm, 38400 for lstm
        
        # Spectrogram + MFCC  
        self.post_spec_mfcc_att_dropout = nn.Dropout(p=0.1)
        self.post_spec_mfcc_att_layer = nn.Linear(256, 993) # 9216 for cnn, 32768 for ltsm s, 65536 for lstm l
    ...
    def forward(self, audio_spec, audio_mfcc, audio_wav):      
        ...
        audio_mfcc1, _ = self.lstm_mfcc(audio_mfcc) # [batch, 300, 512]  
        ...
        audio_mfcc_ = torch.flatten(audio_mfcc1, 1) # [batch, 153600]  
        ...
        # wav2vec 2.0 
        audio_wav_origin = self.wav2vec2_model(audio_wav.cuda()).last_hidden_state # [batch, 149, 768] 
        audio_wav = torch.matmul(audio_spec_mfcc_att_p, audio_wav_origin) # [batch, 1, 768] 
        ...
        audio_wav_origin = audio_wav_origin.reshape(audio_wav_origin.shape[0], -1)
        output = {
            'F1': audio_wav_p,
            'F2': audio_att_1,
            'F3': audio_att_2,
            'F4': output_att,
            'M': output_att,
            'audio_wav_org' : audio_wav_origin,
            'audio_wav_att' : audio_wav,
            'attention_weight' : audio_spec_mfcc_att_p,
            'audio_mfcc' : audio_mfcc_
        }            
