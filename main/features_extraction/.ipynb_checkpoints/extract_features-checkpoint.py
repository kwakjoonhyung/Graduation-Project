...
def main(args):
    ...
    if dataset == 'OUR':
        ...
        # Some publication works combined 'happy' and 'excited' into one 'happy' class,
        #   enable below for the 5531 dataset
        emot_map = {'Angry': 0, 'Sad':1, 'Happy':2, 'Neutrality':3, 'Anxious':4, 'Hurt':5, 'Embarrassed':6}
        include_scripted = True 
        #Initialize database
        database = SER_DATABASES[dataset](dataset_dir, emot_map=emot_map, 
                                        include_scripted = include_scripted)
    ...
    classes = database.get_classes()
    n_class=len(classes)
    class_dist= np.zeros((n_class),dtype=int)

    cnt = Counter(features_data["seg_label"])
    for item in cnt.items():
        class_dist[item[0]] = item[1]

    data_shape = str(features_data["seg_spec"].shape) if not mixnoise else str(features_data["seg_spec"][0].shape)
    
    df = {"shape (N,C,F,T)": [data_shape]}
    for c in range(class_dist.shape[0]):
        df[classes[c]] = [class_dist[c]]
...
def parse_arguments(argv):
    ...
    parser.add_argument('--dataset', type=str, default='OUR',
        help='Dataset to extract features. Options:'
             '  - IEMOCAP (default)'
             '  - EMODB')
    parser.add_argument('--dataset_dir', type=str, default='./test1',
        help='Path to the dataset directory.')
    ...
    parser.add_argument('--segment_size', type=int, default=1987,
        help='Size of each features segment')
...
if __name__ == '__main__':
    args = parse_arguments(sys.argv[1:])
    args.dataset = 'OUR'  # 'IEMOCAP'을 'OUR'로 변경
    main(parse_arguments(sys.argv[1:]))